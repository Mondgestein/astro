 ...
 ; Eric Auer (mceric@users.sourceforge.net), 2001-2009.
...
staknesterr	db "LBACACHE: int stack nest!? otherss=",0

flopfreakerr	db "LBACACHE flush: floppy wrap" ; contiues on next line
axmsg		db " AX=",0
ah2msg		db " AH new.old=",0
...
busyfun	db 0	; busy with which function?
...
carefordrive:
	cmp ah,0x50	; send EDD 3.0 packet command (USB, FireWire)
	jz short t_old_	; ignore 13.50, other func use it internally!
			; (above two lines: simple USB fix 1/2009)
	...
	jz care_stack
		...
		call meep
		pop ax			; AH: new function called
		push ax
		mov al,[cs:busyfun]	; which function already busy?
		push word ah2msg
		call meep
		pop ax
...
care_stack:
	...
	mov [cs:busyfun],ah	; remember which function we are busy with
...
no_geo_chg:
	...
	cmp ah,0x50		; <- AH
	ja short fishyfishy	; stop cache completely any undocumented stuff
...
i13retf:	; must use THIS to do the local stack handling properly!
	...
	mov byte [cs:busyfun],255	; mark as no function busy
	pushf
