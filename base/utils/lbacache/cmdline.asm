...
; GPL 2 software by Eric Auer <eric@coli.uni-sb.de> 2001-2004
	...
	;     Alternative Syntax 2009: DRV NONE
...
cldrvsel:			; cache only drives in (list)
	...
	jnz cldslp
clnodr:	add si,4+1		; skip over "NULL" plus one more
				; (because we have a dec si below)
	jmp short cldsdone

cldslp:				; add drives from user list
	cmp eax,'NONE'		; alternative to NULL keyword: NONE
	jz clnodr
	mov al,[cs:si]
	inc si			; parse on
	cmp al,' '
...
clnodriveatall:
	mov si,clnodrvmsg
		call strtty	; DRV NULL or DRV NONE selected...
...
; ---	db "Load options:",13,10
	...
	db "  DRV NONE  Use this to suppress harddisk caching completely.",13,10
